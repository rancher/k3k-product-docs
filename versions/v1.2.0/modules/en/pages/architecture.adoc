= Architecture
:description: Learn about K3s virtual cluster architecture, including shared mode, Virtual HCP, and how K3s functions as the control plane
...
In shared mode, specific resources must be synchronized between the virtual cluster (child) and the host cluster (parent) to allow workloads to execute correctly. This synchronization is handled under the hood by the K3k Virtual Kubelet.

*Synchronized Resources*

By default, the following resources created in the virtual cluster are automatically synced to the host cluster:
...
Additionally, synchronization for the following resources is supported but disabled by default:
...
You can configure resource synchronization within your virtual cluster's Cluster Custom Resource (CR) under the `spec.sync` field. Each supported resource type includes a selector field to filter which resources get synced by label.

*Storage and Persistent Volume Claims (PVCs)*

When a user creates a PVC inside the virtual cluster, the PVC is automatically synced to the host cluster. The process works as follows:

*StorageClass Dependency*: If the PVC in the virtual cluster references a StorageClass, that StorageClass must already exist on the host cluster. If it does not exist on the host side, the PVC fails to bind to a dynamically created PV. 

[NOTE]
====
Currently, K3k does not sync StorageClasses between the host and virtual cluster, though this behavior is planned for a future release.
====

*Pseudo-Volume Creation*: When the PVC successfully syncs to the host cluster, the k3k-kubelet automatically generates a "pseudo" PersistentVolume (PV) inside the virtual cluster.

*Pod Scheduling*: The pseudo PersistentVolume is bound to the virtual cluster's PVC, enabling the volume to be ready for the consumer pods to be scheduled immediately. However, the physical volume provisioning happens entirely on the host cluster.

For more information, see xref:./resource-sync.adoc[Resource Sync].
...
Pod Security Admission configured _inside_ a virtual cluster is administered by the owner of that virtual cluster, who can relax or remove it. It restricts the workloads of that cluster; it does not protect the host cluster from the owner of the virtual cluster.
...
Runtimes such as link:https://katacontainers.io/[Kata Containers], which runs each pod in a lightweight VM, or link:https://github.com/nestybox/sysbox[Sysbox] provide a kernel or hardware boundary between the virtual cluster and the host node. Support for these runtimes is experimental; see xref:./how-tos-for-admin/custom-runtime-class.adoc[Using a Custom Container Runtime] for a full setup walkthrough.
