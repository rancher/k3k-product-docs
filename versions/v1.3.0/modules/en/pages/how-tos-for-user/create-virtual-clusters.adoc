...
:description: Create a virtual cluster in K3K using Custom Resource Definitions or the K3K CLI to configure storage, persistence, high availability, and specific modes like
...
== Use Case: Create a Virtual Cluster with a specific server node affinity
...
kind: Cluster
metadata:
  name: k3kcluster-server-affinity
spec:
  mode: virtual
  serverAffinity:
    nodeAffinity:
...
== Use Case: Create a Virtual Cluster with a specific agent pod affinity
...
kind: Cluster
metadata:
  name: k3kcluster-agent-affinity
spec:
  mode: virtual
  agentAffinity:
    podAffinity:
