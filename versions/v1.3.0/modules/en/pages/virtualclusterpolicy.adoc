...
:description: Learn how to define and enforce consistent configurations, security settings, and resource management rules for virtual clusters and Namespaces
...
=== Setting Node and Inter-pod Affinity

You can set the node and inter-pod affinity/anti-affinity for both the server pods and the agent pods. By setting the `DefaultAgentAffinity` and `DefaultServerAffinity` fields, you can constrain which nodes your pods can be scheduled on.

The value used is the same as the `spec.affinity` field from pods.

If set, this value overrides any value set on the cluster CR directly.

For more information, see k8s docs: https://kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/#affinity-and-anti-affinity
