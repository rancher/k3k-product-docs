...
:description: Expose the Kubernetes API for virtual clusters to external networks by creating services like LoadBalancer or NodePort and configuring Ingress resources
...
[NOTE]
====
To have K3k manage the exposed Service directly instead (including custom annotations, e.g. for cloud load balancer configuration), use `spec.expose.loadBalancer` / `spec.expose.nodePort` with `spec.expose.annotations`. See xref:/advanced-usage.adoc#_expose[Advanced Usage: expose].
====
