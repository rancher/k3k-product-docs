...
. Utilisez le chemin `/var/lib/rancher/k3s/server/tls k3k/custom-certs/` pour créer les clusters virtuels en 2 modes{nbsp}:
+
* Mode virtuel 
* Mode partagé
...
. Générez des certificats personnalisés via un script{nbsp}:
+
----
...
. Vérifiez les certificats sur le pod du serveur{nbsp}:
+
----
...
. Copiez et vérifiez les certificats dans le répertoire local{nbsp}:
+
----
...
. Vérifiez le cluster k3k nouvellement créé{nbsp}:
+
----
